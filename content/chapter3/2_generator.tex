...
First, distinct values of actual data are aggregated, and a binary mask of differences between clean and dirty datasets is created.
...
Existing missing value and typo frequencies are scaled by scaling factor. Additionally, a specific number of new unique values is generated, this number is estimated as described in Scaling up.
...
For each unique error type a slice of indices is extracted and then applied to the scaled dataset.
...
Outliers are also introduced to columns in parallel manner, Algorithm~\ref{alg:local_generator}.
...
Another limitation is the in-memory computation. This means that data can not be scaled above aggregated memory of a single machine.
