\begin{algorithm}[!t]
    ...
    %   \FOR{$col$ \textbf{in} $\text{data.columns}$} {
    %     \STATE $dist\_changes, col\_name, \mat{updated\_col} \leftarrow \textsc{add\_outliers}($
    %   \STATE $\ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \mat{data}, \mat{err\_dist}, \text{new\_err\_dist.outlier\_indices})$ 
    %     \STATE $\mat{new\_err\_dist}.\textsc{update}(dist\_changes, col\_name)$ 
    %     \STATE $\mat{data}[col] \leftarrow \mat{updated\_col}$ 
    %   }\ENDFOR 
...
\begin{algorithm}[!t]
    ...
    \begin{algorithmic}[1] 
	  ...
	  \COMMENTLINE{e) modify column with errors}
